...
(function () {
    ...
    Markdown.Converter = function () {
        ...
        var g_html_blocks;
        var g_list_level;
        this.makeHtml = function makeHtml (text) {
            if (g_urls) {
        ...
        function _RunSpanGamut(text) {
            ...
            text =
                options._DoItalicsAndBold ? options._DoItalicsAndBold(text) : _DoItalicsAndBold(
                    text);
            text = text.replace(/  +\n/g, " <br>\n");
        ...
        function _DoItalicsAndBold(text) {
            text =
                text.replace(/([\W_]|^)(\*\*|__)(?=\S)([^\r]*?\S[\*_]*)\2([\W_]|$)/g,
                             "$1<strong>$3</strong>$4");
            text =
                text.replace(/([\W_]|^)(\*|_)(?=\S)([^\r\*_]*?\S)\2([\W_]|$)/g, "$1<em>$3</em>$4");
            return text
        }
        ...
        var charInsideUrl = "[-A-Z0-9+&@#/%?=~_|[\\]()!:,.;]", charEndingUrl = "[-A-Z0-9+&@#/%=~_|[\\])]", autoLinkRegex = new RegExp('(="|<)?\\b(https?|ftp)(://'
                                                                                                                                      + charInsideUrl
                                                                                                                                      + "*"
                                                                                                                                      + charEndingUrl
                                                                                                                                      + ")(?=$|\\W)", "gi"), endCharRegex = new RegExp(charEndingUrl, "i");
...
(function () {
    ...
    Markdown.Extra.init = function (converter, options) {
        ...
        if (contains(options.extensions, "all")) {
            options.extensions =
                ["tables", "fenced_code_gfm", "def_list", "attr_list", "footnotes", "smartypants",
    ...
    Markdown.Extra.prototype.doFootnotes = function (text) {
        ...
        text = text.replace(/\[\^(.+?)\]/g, function (wholeMatch, m1) {
            ...
            var html = '<a href="#fn:' + id + '" id="fnref:' + id
                       + '" title="See footnote" class="footnote">' + footnoteCounter + "</a>";
    ...
    Markdown.Extra.prototype.printFootnotes = function (text) {
        ...
        for (var i = 0; i < self.usedFootnotes.length; i++) {
            ...
            text +=
                '<li id="fn:' + id + '">' + formattedfootnote + ' <a href="#fnref:' + id
        ...
        text =
            text.replace(/(?:^|\n)```([^`\n]*)\n([\s\S]*?)\n```[ \t]*(?=\n)/g,
                         function (match, m1, m2) {
                             ...
                             var preclass = self.googleCodePrettify ? ' class="prettyprint hljs-dark"' : "";
                             var codeclass = "";
                             if (language) {
    ...
    Markdown.Extra.prototype.applyPants = function (text) {
        ...
        text =
            text.replace(/(\s|&nbsp;|--|&[mn]dash;|&\#8211;|&\#8212;|&\#x201[34];)'(?=\w)/g,
                         "$1&#8216;");
...
(function () {
    ...
    function initMarkdownExtra() {
        ...
        if (markdownExtra.config.intraword === true) {
            var converterOptions = {
                _DoItalicsAndBold: function (text) {
                    text =
                        text.replace(/([^\w*]|^)(\*\*|__)(?=\S)(.+?[*_]*)(?=\S)\2(?=[^\w*]|$)/g,
                                     "$1<strong>$3</strong>");
                    text =
                        text.replace(/([^\w*]|^)(\*|_)(?=\S)(.+?)(?=\S)\2(?=[^\w*]|$)/g,
                                     "$1<em>$3</em>");
        ...
        converter.hooks.chain("postConversion", function (text) {
            ...
            text = text.replace(/<li>(<p>)?\[([ xX]?)\] /g, function (matched, p, b) {
                p || (p = '');
                return !(b == 'x' || b == 'X') ? '<li class="m-todo-item m-todo-empty">' + p
                                                 + '<input type="checkbox" /> '
                    : '<li class="m-todo-item m-todo-done">' + p
    ...
    function initToc() {
        ...
        toc.convert = function (htmlParsed) {
            var tocExp = new RegExp("^\\s*" + toc.config.marker + "\\s*$");
            var htmlToc = buildToc(htmlParsed);
            // Replace toc paragraphs
            _each(htmlParsed.getElementsByTagName('p'), function (elt) {
    ...
    function initUml() {
        ...
        umlDiagrams.convert = function (target, callback) {
            ...
            var sequenceElems = previewContentsElt.querySelectorAll(
                '.prettyprint > .language-sequence');
                ...
                _each(flowElems, function (elt) {
                    try {
                        ...
                        chart.drawSVG(containerElt,
                                      JSON.parse(umlDiagrams.config.flowchartOptions));
                    }
                    catch (e) {
                        console.error(e);
                    }
...
function md2Html (mdText) {
    var htmlParsed = Markdown.Extra.converter.makeHtml(mdText);
    var toElem = document.createElement('div');
